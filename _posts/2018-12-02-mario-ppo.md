...
Code is not yet available on github, but will be some time this week.
...
Now that we understand how the neural network is able to deal with pixelated inputs, we will move onto the feed-forward (dense) portion of our model - it splits into a value estimation stream and a policy stream. Below we show the implementation in TensorFlow (although there is a slight enhancement that I will mention at the end of the blog):
...
```python
  ...
  # Input placeholders
  inputs = tf.placeholder(shape = [None, image_height, image_width, image_channels], dtype = tf.float32, name = "StateInput")
  previous_actions = tf.placeholder(shape = [None, timesteps * output_dimension], dtype = tf.float32)
  training = tf.placeholder(shape = None, dtype = tf.bool)  
  ...
  weights_hidden = tf.Variable(tf.random_normal([dimensionality, n_hidden], stddev = tf.sqrt(2/(dimensionality + n_hidden))), trainable = trainable, name = "WeightsHidden")
  bias_hidden = tf.Variable(tf.zeros([1,n_hidden]) + 0.01, trainable = trainable, name = "BiasHidden")
  hidden_layer = tf.nn.tanh(tf.matmul(conv2_flat, weights_hidden) + bias_hidden, name = "HiddenLayer")
  concat_hidden_layer = tf.concat([hidden_layer, previous_actions], 1)

  hidden_layer_normalized = tf.layers.batch_normalization(concat_hidden_layer, training = training, trainable = trainable)

  # Policy Output
  weights_policy = tf.Variable(tf.random_normal([n_hidden + timesteps * output_dimension, output_dimension], stddev = tf.sqrt(2/(n_hidden + timesteps * output_dimension + output_dimension))), trainable = trainable, name = "WeightsPolicy")
  ...
  # Value Output
  weights_value = tf.Variable(tf.random_normal([n_hidden + timesteps * output_dimension, 1], stddev = tf.sqrt(2/(n_hidden + timesteps * output_dimension + output_dimension))), trainable = trainable, name = "WeightsValue")
