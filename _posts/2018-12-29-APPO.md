...
2. Large parameter updates during the start of the training round can ruin training for the rest of the round
3. Using a ratio to measure divergence from the old policy handicaps updates for low probability actions

Before diving into each of these points, I want to define a word I will be using going forward: **training round** is defined as the series of updates to our policy network after collecting a certain number of experiences. For example, we can define one training round to be 4 epochs after playing two episodes of a game.

### 1) Reactive vs Proactive

Clipping only takes effect after the policy gets pushed outside the range. As such, it is a reactive algorithm because it only restricts movement once the policy has already moved outside of the specified bounds. This means that PPO does not ensure the new policy is proximal to the old policy because $$r_t(\theta)$$ can easily move well below $$1 - \epsilon$$ or above $$1 + \epsilon$$. As we will see later in this post, by using the accelerated gradient concept behind NAG, we can design a proactive algorithm which anticipates how much the policy will move. We can then use the anticipated move to better control our policy and keep it roughly within the bounds.

### 2) Large Parameter Updates

Based on our previous point, if our policy gets knocked out of the range, then the gradient for that $$(s,a)$$ pair becomes 0. This is important because that means that once we get knocked out of the range, we cannot re-enter during the same training round. We are only able to continue training that $$(s,a)$$ pair on the next training round when $$\pi_{\theta_\text{old}}$$ gets reset.

For example, let's say we have 20 batches per epoch to train on for a given round. On the first batch, if we take an action that gives us a big negative advantage (i.e. we died in a game), then this could easily push us outside of the range $$r_t(\theta) < 1 - \epsilon$$. This means that the gradient becomes 0 for that $$(s,a)$$ pair. Now imagine if we died because of randomness embedded in the game, and the action we took in that state was actually a good decision. For example, in the following batches that same $$(s,a)$$ pair yielded a positive advantage 10 times. That's devastating because we won't be able to move our parameters in the correct direction since the gradient is zero.

Two negative consequences can happen as a result:

1. Training progresses more slowly if we have to wait for the next round to make the update that we should have been able to make in this round
2. If you don't keep an experience replay, you might not revisit that $$(s,a)$$ pair for a while since the probability of taking that action is now much lower

Before moving on, I want to note that it is possible to re-enter the range $$1 - \epsilon < r_t(\theta) < 1 + \epsilon$$ after getting knocked out. This can happen because the neural network weights change in the proceeding mini-batches for other $$(s,a)$$ pairs. However, you should not count on this, as it will not always happen.

### 3) Ratio vs Absolute Difference
...
Let's deal with the ratio first. In order to get rid of the denominator problem, we define $$\hat{r}_t(\theta)$$ as an absolute difference between the new policy and the old policy:
